...
class Doodler:
    ...
    def __init__(
        ...
        self.polygon_sizes = (
            poly  # possible sizes (# sides = # angles) for random polygons
        )
    ...
    def gen_fractional_image_set(self, size, fillfrac=0.5):
        ...
        unfilled_images = np.random.choice(
            self.unfilled_images, size - filled_count, replace=True
        )
    ...
    def check_availability(self, center, half_width, half_height):
        ...
        for dc in range(-hw, hw + 1):
            for dr in range(-hh, hh + 1):
                ...
                c = center[1] + dc
                if (
                    not (0 <= c < self.canvas_cols)
                    or not (0 <= r < self.canvas_rows)
                    or self.cell_covered(r, c)
                ):
                    return False
    ...
    def gen_random_width(self, minfrac=0, maxfrac=1):
        range = [
            max(1, math.floor(minfrac * self.max_width)),
            math.ceil(maxfrac * self.max_width),
        ]
        return random.randint(*range)

    def gen_random_height(self, minfrac=0, maxfrac=1):
        range = [
            max(1, math.floor(minfrac * self.max_height)),
            math.ceil(maxfrac * self.max_height),
        ]
        return random.randint(*range)
    ...
    def doodle(self, fill_pred):
        cas = (
            ...
            else self.gen_canvas(background=self.background)
        )
        for r in range(self.canvas_rows):
            for c in range(self.canvas_cols):
    ...
    def gen_bar_predicate(self, r1, c1, r2, c2):
        ...
        if r1 == r2:
            ...
            slope = (
                p2[1] - p1[1]
            ) / dx  # dx will not be zero, since that's covered by the c1 == c2 case
    ...
    def gen_spiral(self, center, w2, h2):
        ...
        cp0 = self.get_cell_center(
            *center
        )  # gets the (x,y) coords, both in range [0,1]
    ...
    def gen_polygon(self, center, w2, h2, n=5, regular=False):
        ...
        pts = gen_convex_polygon(
            n, xr=(0, 1), yr=(0, 1), plot=False, vect=False, regular=regular
        )  # all coords in range [0,1]
        rmin = center[0] - h2
        cmin = center[1] - w2
        dr = 2 * h2 + 1
        dc = 2 * w2 + 1
        pts2 = [
            scale_pt(p) for p in pts
        ]  # The pts should now be in doodler coordinates.
        pairs = [
            [pts2[i], pts2[i + 1]] for i in range(len(pts2) - 1)
        ]  # Gen pairs of adjacent points
    ...
    def gen_image(
        self, types=["ball"], wr=None, hr=None, figcount=1, fillfrac=None, poly=None
    ):
        if self.multi:
            self.clear_cover_map()
    ...
    def consider_noise(self):
        ...
        def fp(r, c):
            ...
            return (
                not (base_truth)
                if random.uniform(0, 1) <= self.noise_bias
                else base_truth
            )
    ...
    def gen_random_case(
        self,
        image_types=_doodle_image_types_,
        wr=None,
        hr=None,
        flat=True,
        label=None,
        ...
        a, count = self.gen_image(
            types=itypes, wr=wr, hr=hr, figcount=figcount, fillfrac=fillfrac, poly=poly
        )
        if label is None:
            label = self.gen_label(
                ...
                figcount=figcount,
                fillfrac=fillfrac,
                fillcount=(count if fcount else None),
                tlen=tlen,
                poly=poly,
            )
    ...
    def gen_label(
        ...
        if poly is not None:
            ...
            return (
                fillcount if fillcount is not None else round(fillfrac * (tlen - 1))
            )  # label between 0 and tlen-1
        else:
            return (
                figcount if self.multi else all_image_types.index(curr_image_types[0])
            )
    ...
    def gen_random_mono_shape_cases(
        self,
        count,
        image_types=_doodle_image_types_,
        figcount=(1, 5),
...
def gen_standard_cases(
    count=10,
    rows=50,
    cols=50,
    types=_doodle_image_types_,
    wr=[0.2, 0.5],
    hr=[0.2, 0.4],
    noise=0,
    cent=False,
...
def gen_doodle_cases(
    ...
    if mono:
        cases = d.gen_random_mono_shape_cases(
            count, image_types=imt, figcount=fc, flat=flat, wr=wr, hr=hr
        )
    else:
        cases = d.gen_random_cases(
            count,
            image_types=imt,
            flat=flat,
            wr=wr,
    ...
    if d4:
        fulldims.append(1)  # Specialized for DualNets (see neural/deepauto.py)
    ...
    if auto:
        ...
        if one_hots:  # return one-hot vectors for the targets
            ...
            if mono:
                vsize *= len(
                    imt
                )  # For mono runs, vector size = # shapes x # cardinalities
            targets = np.array(
                [
                    np.array(integer_to_one_hot(targ, vsize, zerofill=zfill))
                    for targ in targets
                ]
            )
...
def gen_convex_polygon(n, xr=(0, 1), yr=(0, 1), plot=True, vect=False, regular=False):
    ...
    def scale_pt(pt):
        return [
            xr[0] + dx * (pt[0] - minptx) / ptdx,
            yr[0] + dy * (pt[1] - minpty) / ptdy,
        ]
    ...
    if plot or not (vect):
        ...
        pts = (
            []
        )  # If the algorithm works properly, the first pt gets added at the end of the cycle.
        for v in vectors:
            pt = (pt[0] + v[0], pt[1] + v[1])
            pts.append(pt)
        minptx, minpty, ptdx, ptdy = calc_pt_extrema(
            pts
        )  # Bind 4 variables used by scale_pt
...
def graphics_start_interactive_mode():
    PLT.ion()
...
def quickplot_matrix(
    ...
    axes.imshow(
        X, cmap=colormap, interpolation="nearest", aspect="auto", origin="lower"
    )
